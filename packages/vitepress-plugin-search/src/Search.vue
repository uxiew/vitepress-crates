...
const result = computed(() => {
  if (searchTerm.value) {
    ...
    for (var i = 0; i < searchResults.length; i++) {
      ...
      var link = item["l"];
      var anchor = item["a"];
      if(options.value?.separator) link = link.split(" ").join(options.value?.separator)
      ...
      <Teleport to="body">
        <div v-show="open" class="VPPluginSearch-modal-back" @click="open = false">
          <div class="VPPluginSearch-modal" @click.stop>
            ...
            <img v-if="options?.showLogo" class="VPPluginSearch-flex-logo" src="./flex-logo.svg" alt="flex logo"/>
          </div>
        </div>
      </Teleport>
    </ClientOnly>
    <div id="docsearch" @click="openSearch()">
      ...
      >
        <span class="DocSearch-Button-Container">
          ...
          <span class="DocSearch-Button-Placeholder">{{options?.buttonLabel}}</span>
        </span>
